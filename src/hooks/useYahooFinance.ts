...
import { fetchQuotes, getCachedPrices, BIST_SYMBOLS } from '@/services/yahooFinance';
...
export function useYahooFinance() {
  ...
  const query = useQuery<BistStock[], Error>({
    queryKey: ['yahoo-prices', BIST_SYMBOLS.join(',')],
    queryFn: () => fetchQuotes(BIST_SYMBOLS),
  ...
  // Determine if showing cached data (when API failed)
